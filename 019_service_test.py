...
def get_019_body(accountDebit, accountCredit):
    current_body = data.service_019_body.copy()
...
def positive_assert_amount_with_accountDebit_and_accountCredit(accountDebit, accountCredit):
    service_019_body = get_019_body(accountDebit, accountCredit)
    payment_response = esb_request.service_post(service_019_body)
...
# Класс с тестами
@allure.suite("(019 сервис) Взимание комиссии со счета клиента")
class TestAmountSuite:
    ...
    @allure.title("Взимание комиссии со счета клиента (KGS)")
